...
实现层面按领域边界拆分，而不是把所有 MVP 能力放进一个大模块。案例管理是底座，LLM 增强、向量索引、CBR 检索推荐、反馈闭环和前端后台围绕它分层协作。
...
- `cbr-retrieval-recommendation`：结构化过滤、向量召回、CBR 重排和推荐解释。
...
- CBR 检索推荐依赖案例、LLM 增强和向量索引，但核心数据模型不能反向依赖 CBRKit。
