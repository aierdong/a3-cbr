...
- **检索层**：由 CBRKit 承担案例推理编排，结合结构化过滤、向量召回、重排和结果组装。
...
- **CBR Orchestration**：CBRKit，作为可替换的案例推理编排层。
...
- **CBRKit 不绑定核心模型**：案例数据模型和持久化设计不能与 CBRKit 强绑定，保留未来替换检索编排层的空间。
...
后端 API 应围绕领域边界组织：案例管理、LLM 增强、向量索引、CBR 检索推荐、反馈记录和后台页面消费。共享契约需要优先稳定，特别是案例字段、案例状态、embedding 输入文本、检索过滤字段、推荐解释结构、反馈记录与检索日志之间的关联。
