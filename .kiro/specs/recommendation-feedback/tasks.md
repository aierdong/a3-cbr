...
- [ ] 1.2 建立推荐反馈存储结构
  ...
  - 增加同一用户同一反馈目标唯一约束（PostgreSQL 15+ 使用 `UNIQUE NULLS NOT DISTINCT`，15 以下使用两个部分唯一索引），以及运行、推荐项、案例、用户、时间和状态查询索引。
...
- [ ] 3.1 (P) 实现反馈删除
  - 支持按 `case_id`、`recommendation_run_id`、`recommendation_item_id` 删除反馈。
  - 返回删除数量。
  - 完成后案例删除可触发反馈删除，定时清理程序可删除无效记录。
  - _Requirements: 上游推荐记录生命周期_
  - _Boundary: FeedbackRepository, FeedbackService_
  - _Depends: 2.3_

- [ ] 3.2 (P) 实现反馈明细过滤查询
  - 支持按推荐运行、推荐项、案例、用户、时间范围和有用性过滤。
  - 查询结果返回反馈结果、推荐引用字段和审计字段。
...
- [ ] 4.2 暴露反馈删除 API
  - 提供反馈删除入口，支持按 `case_id`、`recommendation_run_id`、`recommendation_item_id` 删除反馈。
  - 返回删除数量。
  - 完成后案例删除可触发反馈删除，定时清理程序可删除无效记录。
  - _Requirements: 上游推荐记录生命周期_
  - _Boundary: FeedbackRouter_
