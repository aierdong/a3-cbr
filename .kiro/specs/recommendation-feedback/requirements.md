...
`recommendation-feedback` 为门店、督导和后台使用者提供推荐结果反馈记录能力。用户查看 `cbr-retrieval-recommendation` 返回的相似案例后，可以对一次整体推荐（即一次推荐运行）或其中某条推荐项标记**有用性**并可选填写**反馈备注**；系统需要保存查询哈希、可选脱敏查询摘要、过滤条件、命中案例、推荐运行标识、推荐项标识和上述反馈内容。
...
术语说明：
- `整体推荐`：一次查询触发的一整组推荐结果，对应上游一个 `recommendation_run_id`。
- `推荐运行` / `run 级` / `运行级`：与“整体推荐”同义，强调这是对整组推荐结果的反馈，而不是单个结果。
- `推荐项` / `item 级`：整体推荐中的单条结果，对应一个 `recommendation_item_id`。

## Boundary Context

- **In scope**: 推荐反馈实体、有用/无用、反馈备注（可选）、查询哈希、可选脱敏或截断查询摘要快照、过滤条件快照、命中案例关联、整体推荐（推荐运行）和推荐项关联、反馈提交 API、反馈查询 API、基础统计查询、反馈删除 API、无效反馈清理、隐私与失败边界。
...
**Objective:** As a 门店用户或督导, I want 对一次整体推荐（推荐运行）或具体推荐项提交反馈, so that 团队可以知道推荐结果是否真正有用。
...
1.3 When 反馈仅评价整次整体推荐（推荐运行）, the 推荐反馈服务 shall 允许 `recommendation_item_id` 为空，并将反馈标记为运行级反馈（即整体推荐级反馈）。
...
3.1 When 反馈被保存, the 推荐反馈服务 shall 保留推荐运行标识和推荐项标识；查询哈希、应用过滤条件、候选数量和返回数量等查询上下文信息通过关联查询 `recommendation_runs` 表获取，不在反馈表中冗余存储。
3.2 When 反馈关联推荐项, the 推荐反馈服务 shall 保留案例标识和推荐项标识；排序位置、向量相似度、语义相似度、结构化局部相似度、业务参数分、最终聚合分和解释状态等推荐项详细信息通过关联查询 `recommendation_item_snapshots` 表获取，不在反馈表中冗余存储。
3.3 The 推荐反馈服务 shall 保留反馈创建人标识、创建时间、更新时间和反馈来源渠道。
3.4 The 推荐反馈服务 shall 不把反馈结果写回案例基础字段、向量索引、推荐运行记录或推荐项快照。
3.5 If 上游推荐运行标识或推荐项标识不存在, then the 推荐反馈服务 shall 拒绝保存反馈，并返回上游引用不存在的错误信息。
...
4.2 When 查询单条推荐运行（整体推荐）的反馈, the 推荐反馈服务 shall 返回运行级反馈（整体推荐级）和该运行下各推荐项反馈（item 级）。
...
### Requirement 6: 删除能力与无效反馈清理

**Objective:** As a 系统运营者或上游级联删除协调器, I want 对反馈记录执行可审计删除并定期清理无效引用, so that 反馈数据与上游推荐记录生命周期保持一致。

#### Acceptance Criteria

6.1 When 调用反馈删除接口, the 推荐反馈服务 shall 支持按 `feedback_id`、`case_id`、`recommendation_run_id`、`recommendation_item_id` 至少一种条件删除反馈记录。
6.2 When 执行删除, the 推荐反馈服务 shall 记录删除原因 `reason` 与删除者标识 `requested_by`，并返回稳定删除结果（`success`、删除数量、删除时间）。
6.3 If 删除目标不存在, then the 推荐反馈服务 shall 返回成功响应与 `deleted_count = 0`（幂等删除），而不是返回系统错误。
6.4 While 上游记录被清理或出现悬空引用, the 推荐反馈服务 shall 通过定时清理任务删除无效反馈记录，且不影响反馈提交、查询与统计主流程。
6.5 The 推荐反馈服务 shall 支持手动触发清理并返回清理结果（扫描数量、删除数量、耗时），用于运维排障与一致性验证。
