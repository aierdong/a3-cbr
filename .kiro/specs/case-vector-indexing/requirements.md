...
### Requirement 3: 向量记录与索引状态

**Objective:** As a 后台管理者, I want 每个可索引案例都有清晰的向量记录和状态, so that 前端和下游流程能够判断案例是否可参与候选向量搜索。

#### Acceptance Criteria

3.1 When 案例 embedding 校验通过, the 案例向量索引服务 shall 保存问题侧语义向量、结构化过滤字段和来源版本。
3.2 The 案例向量索引服务 shall 为每个案例最多维护一条向量记录。
3.3 While 刷新或重试任务处于排队、处理中、失败、可重试或成功状态, the 案例向量索引服务 shall 基于任务记录和当前向量对外返回明确索引状态。
3.4 When 案例状态不允许被检索且调用方明确请求删除, the 案例向量索引服务 shall 物理删除向量记录，并通过任务记录保留可审计状态变化。
...
4.1 When 调用方针对案例基础字段、案例状态或可消费的 LLM 派生文本变化手动请求刷新, the 案例向量索引服务 shall 通过比较 `case_updated_at` 和 `enrichment_id` 判断是否需要创建刷新任务。
4.2 When 刷新任务成功完成, the 案例向量索引服务 shall 在事务内删除旧向量（如果存在）并插入新向量。
4.3 If 刷新任务失败且错误可重试, then the 案例向量索引服务 shall 按受控次数记录重试尝试和下一次可重试时间。
4.4 If 刷新任务超过重试上限或错误不可重试, then the 案例向量索引服务 shall 将任务状态标记为失败并保留最近一次向量的可用性说明。
4.5 When 案例被删除、归档或变为不可作为检索来源且调用方明确请求删除, the 案例向量索引服务 shall 物理删除向量记录，停止将该案例作为向量搜索命中返回。
