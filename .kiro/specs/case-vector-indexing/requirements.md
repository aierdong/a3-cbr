...
`case-vector-indexing` 为 A3 案例知识库提供可检索的问题侧语义向量索引能力。该模块消费 `a3-case-management` 的基础问题字段和过滤字段，以及 `llm-case-enrichment` 已校验的问题摘要、结构化建议、标签建议和派生结果状态；在案例创建、更新、删除或状态变化后生成 embedding，并维护可供下游推荐阶段稳定消费的索引状态与候选向量搜索原语。
...
- **In scope**: 问题侧向量化输入文本策略、云端 embedding 调用、案例问题向量记录、索引状态、刷新与重试、删除与状态变化一致性、基础 Top-K 向量搜索原语。
...
3.1 When 案例 embedding 校验通过, the 案例向量索引服务 shall 保存问题侧语义向量、结构化过滤字段、输入版本和索引状态。
3.2 The 案例向量索引服务 shall 为每个案例维护当前有效向量记录，并避免同一输入版本重复发布多条有效向量。
3.3 While 案例向量处于待处理、处理中、失败、已降级、已发布、已过期或已移除状态, the 案例向量索引服务 shall 对外返回明确状态。
3.4 When 案例状态不允许被检索, the 案例向量索引服务 shall 将对应向量标记为不可检索或移除，并保留可审计状态变化。
3.5 The 案例向量索引服务 shall 不把向量、相似度、推荐分值或反馈信息写回 `a3-case-management` 的案例基础字段。

### Requirement 4: 刷新、删除与重试一致性

**Objective:** As a 运维人员, I want 案例内容变化后向量索引可自动刷新并可控重试, so that 索引不会长期落后于案例和 LLM 派生内容。

#### Acceptance Criteria

4.1 When 案例基础字段、案例状态或可消费的 LLM 派生文本发生变化, the 案例向量索引服务 shall 判断现有向量是否过期并创建刷新任务。
...
4.4 If 刷新任务超过重试上限或错误不可重试, then the 案例向量索引服务 shall 将状态标记为失败并保留最近一次有效向量的可用性说明。
4.5 When 案例被删除、归档或变为不可作为检索来源, the 案例向量索引服务 shall 停止将该案例作为向量搜索命中返回。

### Requirement 5: 向量搜索原语

**Objective:** As a CBR 检索推荐实现者, I want 使用稳定的向量搜索原语获取候选案例, so that 推荐流程可在本规格之外完成候选补齐、重排和解释。
...
6.2 The 案例向量索引服务 shall 记录向量任务生命周期，包括排队、处理中、成功、失败、重试、过期和移除。
