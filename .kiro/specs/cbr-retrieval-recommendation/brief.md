...
用户输入新问题后，系统要先按品牌、门店规模、问题类型、标签、案例状态和时间范围等上游已支持条件过滤（如果没有提供过滤条件则跳过此步骤），再进行向量召回，并使用 Reranker（模型 id：`qwen3-reranker-8b`）对召回候选重排，返回 Top-K 相似案例。每条结果都要包含相似度、推荐理由、核心解决步骤和可追溯案例索引。

## Approach

检索推荐作为独立规格，消费案例数据、LLM 派生内容和向量索引。案例推理引擎负责调用 `qwen3-reranker-8b` 完成候选重排并组装结果；核心查询契约、过滤字段、推荐结果结构由系统自身定义。

## Scope

- **In**: 查询输入标准化、基础过滤条件、Top-K 向量召回、`qwen3-reranker-8b` 重排、分值聚合、相似度分值、推荐理由结构、原始案例引用、手动检索入口。
...
推荐结果必须可解释、可追溯，不允许只返回黑盒文本。设计上需要保留编排层的替换空间。
